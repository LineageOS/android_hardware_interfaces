...
MctProtocol::MctProtocol(int* fds, PacketReadCallback event_cb, PacketReadCallback acl_cb)
    : event_cb_(event_cb),
      acl_cb_(acl_cb),
...
size_t MctProtocol::Send(uint8_t type, const uint8_t* data, size_t length) {
    if (type == HCI_PACKET_TYPE_COMMAND) return WriteSafely(uart_fds_[CH_CMD], data, length);
    if (type == HCI_PACKET_TYPE_ACL_DATA) return WriteSafely(uart_fds_[CH_ACL_OUT], data, length);

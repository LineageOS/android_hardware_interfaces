...
interface IModule {
     ...
     * input AudioPort parameter. This device port is further referenced to as "port
     ...
     *
     * The profiles of the template port may adhere to one of the following
     * cases:
     *  - Have a fixed list of profiles that does not depend on the external
     *    device, and instead is fully determined by the capabilities of the
     *    DSP. For example, this is the case of an analog audio device.
     *  - Similar to the previous case, but the resulting list of profiles that
     *    the returned AudioPort instance may be trimmed or expanded depending
     *    on the capabilities of the external device. For example, this may be
     *    the case of a USB audio device connected via SoC DSP.
     *  - Have an empty list of profiles, in this case the list of profiles on
     *    the returned AudioPort will be solely determined by the capabilities
     *    of the external device. This is usually the case with HDMI devices and
     *    USB devices served via tinyALSA directly.
     *
     * In any case, it is always the responsibility of the HAL module to query
     * audio profiles supported by the connected device and return them as part
     * of the returned AudioPort instance. In the case when the HAL is unable to
     * query the external device, an error must be thrown.
     *
     ...
     *  - list of audio profiles that can be used with the connected device;

...
void CameraDevice::sDataCb(int32_t msg_type, const camera_memory_t *data, unsigned int index,
    ...
    if (object->mQDeviceCallback != nullptr) {
        ...
        CameraHeapMemory* mem = static_cast<CameraHeapMemory*>(data->handle);
        object->mQDeviceCallback->QDataCallback((DataCallbackMsg)msg_type, mem->handle.mId, index,
                                                hidlMetadata);
    } else {
        if (object->mDeviceCallback != nullptr) {
            ...
            CameraHeapMemory* mem = static_cast<CameraHeapMemory*>(data->handle);
            object->mDeviceCallback->dataCallback((DataCallbackMsg)msg_type, mem->handle.mId, index,
                                                  hidlMetadata);
        }
    }
}
